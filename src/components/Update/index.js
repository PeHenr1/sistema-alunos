...
function Update() {
  ...
  const handleUpdate = (event) => {
    event.preventDefault();
    axios
      .put(`https://671990d57fc4c5ff8f4dc0cb.mockapi.io/movies/${searchId}`, values)
      .then(() => {
        navigate('/');
      })
  ...
  return (
    <div className="d-flex justify-content-center">
      <div className="w-50 border bg-light shadow px-5 pt-5 pb-5 rounded">
        ...
        {!errorMessage && !showEditForm && renderSearchForm()}
        {!errorMessage && showEditForm && renderEditForm()}
      </div>
    </div>
  );
}
